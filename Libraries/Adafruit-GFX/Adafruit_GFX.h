...
class Adafruit_GFX : public Print 
{
 public:
  ...
  virtual void drawPixel(int16_t x, byte y, uint16_t color) = 0;

  // TRANSACTION API / CORE DRAW API
  // These MAY be overridden by the subclass to provide device-specific
  // optimized code.  Otherwise 'generic' versions are used.
  virtual void startWrite(void);
  virtual void writePixel(int16_t x, byte y, uint16_t color);
  virtual void writeFillRect(int16_t x, byte y, int16_t w, byte h, uint16_t color);
  virtual void writeFastVLine(int16_t x, byte y, int16_t h, uint16_t color);
  virtual void writeFastHLine(int16_t x, byte y, int16_t w, uint16_t color);
  virtual void writeLine(int16_t x0, byte y0, int16_t x1, byte y1, uint16_t color);
  ...
  virtual void
    ...
    drawFastVLine(int16_t x, byte y, int16_t h, uint16_t color),
    drawFastHLine(int16_t x, byte y, int16_t w, uint16_t color),
    fillRect(int16_t x, byte y, int16_t w, byte h, uint16_t color),
    fillScreen(uint16_t color),
    // Optional and probably not necessary to change
    drawLine(int16_t x0, byte y0, int16_t x1, byte y1, uint16_t color),
    drawRect(int16_t x, byte y, int16_t w, byte h, uint16_t color);
  ...
  void
    ...
    fillTriangle(int16_t x0, byte y0, int16_t x1, byte y1, int16_t x2, byte y2, uint16_t color),
...
class GFXcanvas1 : public Adafruit_GFX 
{
 public:
	...
	~GFXcanvas1(void);
	void drawPixel(int16_t x, byte y, uint16_t color);
...
class GFXcanvas8 : public Adafruit_GFX 
{
 public:
	...
	void drawPixel(int16_t x, byte y, uint16_t color);
	void fillScreen(uint16_t color);
	void writeFastHLine(int16_t x, byte y, int16_t w, uint16_t color);
	uint8_t *getBuffer(void);
 private:
	uint8_t *buffer;
};
